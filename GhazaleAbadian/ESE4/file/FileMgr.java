...
public class FileMgr {
   ...
   synchronized void read(Block blk, ByteBuffer bb) {
      try {
         ...
         blockStatsFile.get(blk.fileName()).incrementerBlockRead();
      }
      catch (IOException e) {
   ...
   synchronized void write(Block blk, ByteBuffer bb) {
      try {
         ...
         blockStatsFile.get(blk.fileName()).incrementerBlockWritten();
      }
      catch (IOException e) {
   ...
   private FileChannel getFile(String filename) throws IOException {
      ...
      if ( !this.blockStatsFile.containsKey(filename) ) {
    	  ...
    	  this.blockStatsFile.get(filename).setBlockRead(0);
    	  this.blockStatsFile.get(filename).setBlockWritten(0);
      }
      return fc;
   }
